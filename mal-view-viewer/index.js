export default function define(runtime, observer) {
    ...
    const fileAttachments = new Map([["flare.json", new URL("./generated/parsed.json", import.meta.url)]]);
...
<b style="color: ${colorout};">outgoing</b> and <b style="color: ${colorin};">incoming</b>`
        )});
    main.variable(observer("chart")).define(
        ...
        function (tree, bilink, d3, data, width, id, colornone, line, colorin, colorout) {
            const root = tree(bilink(
                d3.hierarchy(data).sort((a, b) => d3.ascending(a.height, b.height) || d3.ascending(a.data.path, b.data.path))));
            ...
            const node = svg.append("g")
                ...
                .on("mouseover", overed)
                .on("mouseout", outed)
                .on("click", function(event, d) { console.log(event); window.open(d.data.origin); })
...
${d.incoming.length} incoming`));
    ...
    main.variable(observer("bilink")).define("bilink", ["id"], function (id) {
        return (
            function bilink(root) {
                ...
                for (const d of root.leaves()) d.incoming = [], d.outgoing = d.data.subs.map(i => [d, map.get(i)]);
    ...
    main.variable(observer("id")).define("id", function () {
        return (
            function id(node) {
                return `${node.parent ? id(node.parent) + ":" : ""}${node.data.path}`;
            }
        )
    });
